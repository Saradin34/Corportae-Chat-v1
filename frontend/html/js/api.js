...
(function () {
  ...
  const API = {
    ...
    savedChat: () => request("POST", "/chats/saved"),
    ...
    adminCreateContact: (d) => request("POST", "/admin/users/contact", d),
    adminDeleteContact: (id) => request("DELETE", "/admin/users/contact/" + id),
    ...
    adminAdSearchGroups: (q, limit) => request("GET", "/admin/groups/ad/search?q=" + encodeURIComponent(q || "") + "&limit=" + encodeURIComponent(limit || 50)),
  ...
  // ---- Toast helper (global) ----
  function isTechnicalNoise(message) {
  ...
  window.toast = function (message, type) {
    ...
    if (type === "error" && isTechnicalNoise(message)) {
      try { console.warn("Suppressed technical toast:", message); } catch (e) {}
      return;
    }
  ...
  window.addEventListener("error", (e) => {
    if (isTechnicalNoise(e && e.message)) {
      try { console.warn("Suppressed window error:", e.message); } catch (x) {}
      e.preventDefault();
    }
  });
  window.addEventListener("unhandledrejection", (e) => {
    const msg = e && e.reason && (e.reason.message || String(e.reason));
    if (isTechnicalNoise(msg)) {
      try { console.warn("Suppressed promise rejection:", msg); } catch (x) {}
      e.preventDefault();
    }
  });
})();
  ...
  window.uiConfirm = function (message, title) {
    const opts = (title && typeof title === "object") ? title : { title };
    return new Promise((resolve) => {
      ...
      const okClass = opts.danger === false ? "btn-primary inline" : "btn-danger";
      o.innerHTML = `
        <div class="app-dialog" role="dialog" aria-modal="true">
          <h3>${escapeDlg(opts.title || "Подтверждение")}</h3>
          <p>${escapeDlg(message || "")}</p>
          <div class="app-dialog-actions">
            <button class="btn-secondary" id="dlg-cancel">${escapeDlg(cancelText)}</button>
            <button class="${okClass}" id="dlg-ok">${escapeDlg(okText)}</button>
          </div>
        </div>`;
