...
(function () {
  ...
  function presenceClass(u) {
    if (!u || !u.is_active) return "off";
    if (!u.is_online) return "offline";
    if (u.status === "dnd") return "dnd";
    if (u.status === "away") return "away";
    return "online";
  }
  function presenceText(u) {
    if (!u || !u.is_active) return "Заблокирован";
    if (!u.is_online) return "Не в сети";
    if (u.status === "dnd") return "Не беспокоить";
    if (u.status === "away") return "Не на месте";
    return "Онлайн";
  }
  function presenceDot(u) {
    const cls = presenceClass(u);
    return u && u.is_online ? `<span class="online-dot ${cls}"></span>` : "";
  }
  ...
  function handleAdminPresence(data) {
    ...
    if (u) { u.is_online = online; u.status = online ? (data.status || "online") : "offline"; }
    ...
    document.querySelectorAll(`tr[data-user-row="${uid}"]`).forEach((row) => {
      ...
      if (av) {
        ...
        if (online) { if (!dot) { dot = document.createElement("span"); dot.className = "online-dot"; av.appendChild(dot); } dot.classList.toggle("away", data.status === "away"); dot.classList.toggle("dnd", data.status === "dnd"); dot.classList.toggle("online", !data.status || data.status === "online"); }
      ...
      if (badge) {
        ...
        badge.textContent = presenceText(u);
      }
    });
  ...
  function openAddContactModal() {
    ...
    const groupOptions = [`<option value="">— без группы —</option>`].concat(
      (_groupsCache || []).filter((g) => !g.is_default).map((g) => `<option value="${g.id}">${esc(g.name)}</option>`)
    ).join("");
    overlay.innerHTML = `
      <div class="modal">
        <div class="modal-header"><h2>Добавить контакт</h2><button class="modal-close">✕</button></div>
        <div class="modal-body">
          <div class="field"><label>ФИО *</label><input type="text" id="contact-full-name" placeholder="Иванов Иван" autofocus /></div>
          <div class="field"><label>Телефон</label><input type="text" id="contact-phone" placeholder="204" /></div>
          <div class="field"><label>E-mail</label><input type="email" id="contact-email" placeholder="user@company.by" /></div>
          <div class="field"><label>Должность</label><input type="text" id="contact-title" placeholder="Инженер" /></div>
          <div class="field"><label>Кабинет / офис</label><input type="text" id="contact-office" placeholder="401" /></div>
          <div class="field"><label>Группа</label><select id="contact-group">${groupOptions}</select></div>
          <div class="settings-sub">Контакт появится в общей Контактной книге. Логин будет создан автоматически и не будет показываться пользователям.</div>
        </div>
        <div class="modal-footer">
          <button class="btn-secondary" id="modal-cancel">Отмена</button>
          <button class="btn-primary inline" id="contact-save">Добавить</button>
        </div>
      </div>`;
    ...
    document.getElementById("contact-save").addEventListener("click", async () => {
      ...
      const groupVal = document.getElementById("contact-group").value;
      ...
      try {
        await API.adminCreateContact({
          ...
          office: document.getElementById("contact-office").value.trim(),
  ...
  function userRow(u, me) {
    return `<tr data-user-row="${u.id}">
      ...
      <td data-label="Пользователь"><div style="display:flex;align-items:center;gap:8px">
        <div class="avatar sm" style="background:${esc(u.avatar_color)}">${initials(u.full_name || u.username)}${presenceDot(u)}</div>
      ...
      <td data-label="Статус"><span data-user-status class="badge ${presenceClass(u)}">${presenceText(u)}</span></td>
    ...
    overlay.innerHTML = `
      <div class="modal">
        ...
        <div class="modal-body" style="text-align:center">
          <div class="usercard-avatar"><div class="avatar lg" style="background:${esc(u.avatar_color)}">${initials(u.full_name || u.username)}${presenceDot(u)}</div></div>
          <div class="usercard-name">${esc(u.full_name || u.username)}</div>
          <div class="usercard-status">${presenceText(u).toLowerCase()} · ${u.role === "admin" ? "администратор" : "пользователь"} · ${u.auth_source === "ldap" ? "AD" : "локальный"}</div>
  ...
  function openCreateGroup() {
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="ad-import-box">
            ...
            <div class="diag-line ad-search-line">
              <input type="text" id="ad-q" placeholder="Введите название группы AD…" autocomplete="off" />
              <button type="button" class="btn-secondary" id="ad-search-btn">Найти</button>
            </div>
    ...
    const adBtn = document.getElementById("ad-search-btn");
    ...
    async function doSearch(force) {
      ...
      if (!q) { adResults.innerHTML = `<div class="ad-hint">Введите часть названия группы AD</div>`; return; }
      if (!force && q.length < 1) { adResults.innerHTML = ""; return; }
      const runId = ++adRunId;
      adResults.innerHTML = `<div class="ad-hint">Поиск…</div>`;
      if (adBtn) adBtn.disabled = true;
      try {
        await runAdSearch(q, adResults, close, runId, () => adRunId);
      } finally {
    ...
    function scheduleAdSearch() {
      clearTimeout(adTimer);
      adTimer = setTimeout(() => doSearch(false), 350);
    }
    adQ.addEventListener("input", scheduleAdSearch);
    adQ.addEventListener("keyup", (e) => { if (e.key !== "Enter") scheduleAdSearch(); });
    adQ.addEventListener("paste", () => setTimeout(scheduleAdSearch, 0));
    adQ.addEventListener("change", scheduleAdSearch);
    adQ.addEventListener("keydown", (e) => {
      if (e.key === "Enter") { e.preventDefault(); doSearch(true); }
    });
    if (adBtn) adBtn.addEventListener("click", () => doSearch(true));
    setTimeout(() => adQ.focus(), 80);
  }

  async function runAdSearch(q, adResults, closeModal, runId, getRunId) {
    let groups = [];
    try {
      groups = await API.adminAdSearchGroups(q, 50);
    } catch (e) {
      if (getRunId && runId !== getRunId()) return;
