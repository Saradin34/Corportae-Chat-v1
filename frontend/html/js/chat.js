...
(function () {
  ...
  function applyPermsToUI() {
    ...
    // Channel readers (can_post=false) get a read-only banner instead of inputs.
    const isReadOnlyChannel = State.activeChat &&
      State.activeChat.type === "channel" && State.activeChat.can_post === false;
    if (composer) {
      let banner = document.getElementById("channel-readonly");
      if (isReadOnlyChannel) {
        if (composerRow) composerRow.style.display = "none";
        if (!banner) {
          banner = document.createElement("div");
          banner.id = "channel-readonly";
          banner.className = "channel-readonly";
          banner.innerHTML = "📢 Только администраторы канала могут публиковать сообщения";
    ...
    if (composerRow && !isReadOnlyChannel) {
  ...
  // ---------- Drafts (per-chat unsent text, localStorage) ----------
  const Drafts = {
    KEY: "cc_drafts",
    _all() {
    ...
    get(chatId) { return this._all()[chatId] || ""; },
    set(chatId, text) {
      const a = this._all();
      if (text && text.trim()) a[chatId] = text;
      else delete a[chatId];
      try { localStorage.setItem(this.KEY, JSON.stringify(a)); } catch (e) {}
    },
    clear(chatId) { this.set(chatId, ""); },
  };
  ...
  function handleWSMessage(data) {
    switch (data.type) {
      case "new_message": {
        ...
        const mentionedMe = Array.isArray(data.mentions) && data.mentions.indexOf(State.me.id) >= 0;
        if (!mine && !m.is_system) notifyIncoming(m, mentionedMe);
        refreshChatList();
        break;
      }
      case "edit_message": {
      ...
      case "read_receipt": {
        // Another member read up to last_read_message_id → update ✓✓ on my
        // outgoing messages and the per-member read marker.
        if (State.activeChat) {
          const mem = State.activeChat.members.find((x) => x.id === data.user_id);
          if (mem) mem.last_read_message_id = data.last_read_message_id;
          if (data.chat_id === State.activeChatId) updateReadTicks();
        }
        break;
      }
  ...
  function renderLayout() {
    ...
    if (me.role === "admin") items += `<div class="drawer-item" data-action="newchannel"><span class="di-icon">📢</span> Новый канал</div>`;
    items += `
      <div class="drawer-item" data-action="status"><span class="di-icon">🟢</span> Мой статус</div>
  ...
  function handleDrawerAction(a) {
    ...
    if (a === "newgroup") return openNewChatModal("group");
    if (a === "newchannel") return openNewChatModal("channel");
    if (a === "status") return openStatusModal();
  }
  ...
  function renderChatList(chats) {
    ...
    list.innerHTML = chats.map((c) => {
      ...
      <div class="chat-item ${c.id === State.activeChatId ? "active" : ""}" data-chat-id="${c.id}">
        ...
        <div class="chat-meta">
          <div class="chat-top">
            <span class="chat-name">${c.is_pinned ? "📌 " : ""}${c.is_muted ? "🔇 " : ""}${escapeHtml(c.name)}</span>
  ...
  async function doTogglePinChat(chatId) {
    try {
      const r = await API.togglePinChat(chatId);
      const c = State.chats.find((x) => x.id === chatId);
      if (c) c.is_pinned = r.is_pinned;
      window.toast(r.is_pinned ? "Чат закреплён" : "Чат откреплён");
      await loadChats();
    } catch (e) { window.toast(e.message, "error"); }
  }

  async function doToggleMuteChat(chatId) {
    try {
      const r = await API.toggleMute(chatId);
      const c = State.chats.find((x) => x.id === chatId);
      if (c) c.is_muted = r.is_muted;
      window.toast(r.is_muted ? "Уведомления отключены" : "Уведомления включены");
      renderChatList(State.chats);
    } catch (e) { window.toast(e.message, "error"); }
  }

  // ---- Export conversation (TXT / HTML / JSON) ----
  function openExportDialog(chatId) {
    const overlay = document.getElementById("modal-overlay");
    if (!overlay) { quickExport(chatId, "txt"); return; }
    overlay.innerHTML = `
      <div class="modal" style="max-width:380px">
        <div class="modal-header"><h3>Экспорт переписки</h3>
          <button class="modal-close">✕</button></div>
        <div class="modal-body">
          <p style="color:var(--text-secondary);font-size:14px;margin:0 0 14px">
            Выберите формат для сохранения истории сообщений.</p>
          <button class="btn-secondary export-opt" data-fmt="txt" style="width:100%;margin-bottom:8px">📄 Текст (.txt)</button>
          <button class="btn-secondary export-opt" data-fmt="html" style="width:100%;margin-bottom:8px">🌐 HTML (для печати в PDF)</button>
          <button class="btn-secondary export-opt" data-fmt="json" style="width:100%">🧩 JSON (данные)</button>
        </div>
      </div>`;
    overlay.classList.add("show");
    ...
    overlay.querySelectorAll(".export-opt").forEach((b) =>
      b.addEventListener("click", () => { quickExport(chatId, b.getAttribute("data-fmt")); close(); }));
  }

  async function quickExport(chatId, fmt) {
    try {
      window.toast("Готовим экспорт…");
      await API.exportChat(chatId, fmt);
      window.toast("Файл сохранён", "success");
    } catch (e) { window.toast(e.message || "Ошибка экспорта", "error"); }
  }
  ...
  function openChatListMenu(chatId, x, y) {
    ...
    items.push({ act: "pin", icon: "📌", label: chat.is_pinned ? "Открепить" : "Закрепить" });
    items.push({ act: "mute", icon: chat.is_muted ? "🔔" : "🔇", label: chat.is_muted ? "Включить уведомления" : "Отключить уведомления" });
    items.push({ sep: true });
    items.push({ act: "export", icon: "📤", label: "Экспорт переписки" });
    ...
    menu.addEventListener("click", (e) => {
      ...
      else if (act === "pin") doTogglePinChat(chatId);
      else if (act === "mute") doToggleMuteChat(chatId);
      else if (act === "export") openExportDialog(chatId);
    });
    setTimeout(() => {
  ...
  async function openChat(chatId) {
    // Save the unsent draft of the chat we're leaving.
    if (State.activeChatId && State.activeChatId !== chatId) {
      const prevInput = document.getElementById("composer-input");
      if (prevInput && !State.editingId) Drafts.set(State.activeChatId, prevInput.value);
    }
  ...
  function renderChatArea() {
    ...
    const input = document.getElementById("composer-input");
    // Restore the saved draft for this chat (if any, and not while editing).
    if (!State.editingId) {
      const draft = Drafts.get(State.activeChatId);
      if (draft) {
        input.value = draft;
        setTimeout(() => {
    ...
    input.addEventListener("input", () => {
      ...
      input.style.height = Math.min(input.scrollHeight, 120) + "px";
      sendTyping();
      // persist draft (skip while editing an existing message)
  ...
  function updateChatHeader() {
    ...
    if (chat.type === "private") {
      ...
      st.textContent = presenceLabel(other);
      const online = other && other.is_online;
      const manual = other && (State.statusMap[other.id] || other.status);
      st.classList.toggle("online", !!online && manual !== "away" && manual !== "dnd" && manual !== "vacation");
    } else if (chat.type === "channel") {
      st.textContent = chat.members.length + " подписчиков · 📢 канал";
  ...
  // Human-readable presence for a user, honoring manual status (dnd/vacation).
  function presenceLabel(u) {
    if (!u) return "не в сети";
    const manual = State.statusMap[u.id] || u.status || "";
    if (manual === "dnd") return "🔴 Не беспокоить";
    if (manual === "vacation") return "🌴 В отпуске";
    if (!u.is_online) return "не в сети";
    if (manual === "away") return "не на месте";
    return "в сети";
  }
      ...
      <div class="msg-row ${out ? "out" : "in"}" data-msg-id="${m.id}">
        <div class="msg-bubble ${m.is_pinned ? "pinned" : ""}">
          ...
          <div class="msg-footer">
            ...
            ${out && !m.is_deleted && !m.is_system ? `<span class="msg-ticks ${isMessageRead(m) ? "read" : ""}" data-tick="${m.id}">${isMessageRead(m) ? "✓✓" : "✓"}</span>` : ""}
          </div>
          ${actions}
        </div>
      </div>`;
  ...
  async function doSend() {
    ...
    if (atts.length) {
      ...
      input.value = ""; input.style.height = "auto";
      Drafts.clear(State.activeChatId);
      clearAttachments();
  ...
  // Shows a small dropdown of chat members when the user types "@" + prefix.
  const Mention = { open: false, items: [], index: 0, start: -1 };

  function mentionCandidates(prefix) {
    if (!State.activeChat) return [];
    const p = (prefix || "").toLowerCase();
    return (State.activeChat.members || [])
      .filter((m) => m.id !== State.me.id)
      .filter((m) => !p
        || (m.username || "").toLowerCase().startsWith(p)
        || (m.full_name || "").toLowerCase().includes(p))
      .slice(0, 6);
  }

  function maybeShowMentionPicker(input) {
    const isGroup = State.activeChat && State.activeChat.type !== "private";
    if (!isGroup) { closeMentionPicker(); return; }
    const pos = input.selectionStart;
    const upto = input.value.slice(0, pos);
    const m = /(^|\s)@([A-Za-z0-9_.\-]*)$/.exec(upto);
    if (!m) { closeMentionPicker(); return; }
    Mention.start = pos - m[2].length - 1; // index of the "@"
    Mention.items = mentionCandidates(m[2]);
    Mention.index = 0;
    if (!Mention.items.length) { closeMentionPicker(); return; }
    renderMentionPicker();
  }

  function renderMentionPicker() {
    let box = document.getElementById("mention-picker");
    if (!box) {
      box = document.createElement("div");
      box.id = "mention-picker";
      box.className = "mention-picker";
      const composer = document.getElementById("composer");
      if (composer) composer.appendChild(box);
    }
    Mention.open = true;
    box.innerHTML = Mention.items.map((u, i) => `
      <div class="mention-item ${i === Mention.index ? "active" : ""}" data-mi="${i}">
        ${avatarHtml({ url: u.avatar_url, color: u.avatar_color, name: u.full_name || u.username, size: "xs" })}
        <div class="mention-meta"><b>${escapeHtml(u.full_name || u.username)}</b>
    ...
    box.querySelectorAll(".mention-item").forEach((el) =>
      el.addEventListener("mousedown", (e) => {
        e.preventDefault();
        applyMention(parseInt(el.getAttribute("data-mi"), 10));
      }));
  }

  function closeMentionPicker() {
    Mention.open = false; Mention.items = []; Mention.start = -1;
    const box = document.getElementById("mention-picker");
  ...
  // Returns true if the keydown was consumed by the mention picker.
  function mentionKeydown(e) {
    if (!Mention.open) return false;
    if (e.key === "ArrowDown") { e.preventDefault(); Mention.index = (Mention.index + 1) % Mention.items.length; renderMentionPicker(); return true; }
    if (e.key === "ArrowUp") { e.preventDefault(); Mention.index = (Mention.index - 1 + Mention.items.length) % Mention.items.length; renderMentionPicker(); return true; }
    if (e.key === "Enter" || e.key === "Tab") { e.preventDefault(); applyMention(Mention.index); return true; }
    if (e.key === "Escape") { closeMentionPicker(); return true; }
    return false;
  }

  // ---- Read receipts (✓ sent / ✓✓ read) ----
  // A message is "read" when EVERY other member's last_read_message_id is >=
  // the message id. For private chats that's just the single other person.
  function isMessageRead(m) {
    if (!State.activeChat || !m || m.is_system) return false;
    const others = (State.activeChat.members || []).filter((x) => x.id !== State.me.id);
    if (!others.length) return false;
    return others.every((x) => (x.last_read_message_id || 0) >= m.id);
  }
  function updateReadTicks() {
    document.querySelectorAll(".msg-ticks[data-tick]").forEach((el) => {
      const id = parseInt(el.getAttribute("data-tick"), 10);
      const m = State.messages.find((x) => x.id === id);
      if (!m) return;
  ...
  function notifyIncoming(m, mentionedMe) {
    const p = Prefs.all();
    ...
    // A direct @mention always pings (sound), even if muted/viewing — it's
    // addressed specifically to you.
    if (mentionedMe && p.sound) playMessageSound();
    // Sound: play unless the user is actively watching this chat.
    else if (p.sound && !viewingThisChat) playMessageSound();

    // For an @mention, show an in-app toast even when this chat is open.
    if (mentionedMe && viewingThisChat) {
      window.toast("💬 " + (m.sender_name || "Кто-то") + " упомянул(а) вас", "success");
    }
    ...
    // An @mention is shown even if general notifications are off.
    if ((!p.notify && !mentionedMe) || viewingThisChat) return;
    ...
    const title = (mentionedMe ? "💬 Упоминание · " : "") + (m.sender_name || "Новое сообщение");
  ...
  async function openNewChatModal(mode) {
    // Backward compat: a boolean `true` used to mean "group mode".
    if (mode === true) mode = "group";
    if (mode === false || !mode) mode = "private";
    const isChannel = mode === "channel";
    const groupMode = mode === "group" || isChannel;  // both need a name + multi-select
    const title = isChannel ? "Новый канал" : (mode === "group" ? "Новая группа" : "Новый чат");
    const nameLabel = isChannel ? "Название канала" : "Название группы";
    const namePh = isChannel ? "Например: Объявления ИТ-отдела" : "Например: Команда разработки";
    const overlay = document.getElementById("modal-overlay");
    overlay.innerHTML = `
      <div class="modal">
        <div class="modal-header"><h2>${title}</h2><button class="modal-close">✕</button></div>
        <div class="modal-body">
          ${isChannel ? `<div class="channel-hint">📢 В канал пишут только администраторы, остальные участники только читают объявления.</div>` : ""}
          <div id="group-name-field" class="field" style="${groupMode ? "" : "display:none"}">
            <label>${nameLabel}</label>
            <input type="text" id="group-name" placeholder="${namePh}" />
          </div>
          <div class="field"><input type="text" id="user-search" placeholder="${isChannel ? "Добавить подписчиков..." : "Поиск пользователей..."}" /></div>
    ...
    createBtn.addEventListener("click", async () => {
      ...
      try {
        let chat;
        if (isChannel) {
          const name = document.getElementById("group-name").value.trim() || "Канал";
  ...
  // ---------- Status (Не беспокоить / В отпуске) ----------
  const STATUS_LABELS = { "": "Авто (в сети)", dnd: "🔴 Не беспокоить", vacation: "🌴 В отпуске" };
  function openStatusModal() {
    const overlay = document.getElementById("modal-overlay");
    const cur = (State.me && State.me.status) || "";
    const opt = (val, icon, label, desc) => `
      <div class="status-opt ${cur === val ? "active" : ""}" data-status="${val}">
        <span class="status-opt-icon">${icon}</span>
        <div class="status-opt-meta"><b>${label}</b><span>${desc}</span></div>
        ${cur === val ? '<span class="checkmark">✓</span>' : ""}
      </div>`;
    overlay.innerHTML = `
      <div class="modal" style="max-width:380px">
        <div class="modal-header"><h2>Мой статус</h2><button class="modal-close">✕</button></div>
        <div class="modal-body">
          ${opt("", "🟢", "Авто (в сети)", "Статус определяется автоматически")}
          ${opt("dnd", "🔴", "Не беспокоить", "Уведомления приглушены для собеседников")}
          ${opt("vacation", "🌴", "В отпуске", "Показывает, что вас сейчас нет")}
        </div>
      </div>`;
    overlay.classList.add("show");
    const close = () => { overlay.classList.remove("show"); overlay.innerHTML = ""; };
    ...
    overlay.querySelectorAll(".status-opt").forEach((el) =>
      el.addEventListener("click", async () => {
        ...
        try {
          const me = await API.setStatus(s);
          State.me = me; API.Store.setUser(me);
          window.toast("Статус: " + (STATUS_LABELS[s] || "обновлён"), "success");
          close();
        } catch (e) { window.toast(e.message, "error"); }
      }));
  }
  ...
  function linkify(safeHtml) {
    let out = safeHtml.replace(/(https?:\/\/[^\s<]+)/g, '<a href="$1" target="_blank" rel="noopener">$1</a>');
    // Highlight @mentions; emphasize the mention of the current user.
    const myName = (State.me && State.me.username || "").toLowerCase();
    out = out.replace(/(^|\s)@([A-Za-z0-9_.\-]+)/g, (full, pre, name) => {
      const me = name.toLowerCase() === myName ? " mention-me" : "";
      return `${pre}<span class="mention${me}">@${name}</span>`;
    });
    return out;
  }
