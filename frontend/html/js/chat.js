...
(function () {
  ...
  const Prefs = {
    ...
    _defaults: {
      ...
      sound: true,
      notify: true,
      notifyCalls: true,      // incoming/missed call system notifications
  ...
  function applyChatBackground(p) {
    ...
    if (kind === "doodleCats") {
      document.documentElement.style.setProperty("--chat-wallpaper", 'url("/assets/wallpapers/doodle-cats.webp")');
  ...
  function handleWSMessage(data) {
    switch (data.type) {
      ...
      case "read_receipt": {
        if (data.chat_id === State.activeChatId && State.activeChat && State.activeChat.members) {
          const m = State.activeChat.members.find((x) => x.id === data.user_id);
          if (m) m.last_read_message_id = Math.max(m.last_read_message_id || 0, data.last_read_message_id || 0);
          rerenderMessages();
        }
        // Keep list unread counters fresh for the current user too.
        refreshChatList();
        break;
      }
    ...
    app().innerHTML = `
      <div class="app-layout" id="app-layout">
        <aside class="app-rail" id="app-rail" aria-label="Быстрые действия">
          ...
          <button class="rail-btn" id="support-btn" title="Поддержка">${icon("support")}</button>
  ...
  function rsUserHtml(u, offline) {
    ...
    const dot = statusDotHtml(stVal, u.is_online);
    ...
    return `<div class="rs-user ${offline ? "offline" : ""} presence-${statusDotClass(stVal, u.is_online)}" data-uid="${u.id}" data-live-user="${u.id}">
  ...
  function pushDesktopUnreadState(chatUnread, callUnread, flash) {
    ...
    if (D && typeof D.setUnreadState === "function") {
      try { D.setUnreadState(chatUnread, callUnread, !!flash); return; } catch (e) {}
    }
    if (D && typeof D.setUnread === "function") {
      try { D.setUnread((chatUnread || 0) + (callUnread || 0), !!flash); } catch (e) {}
    }
  }
  function updateUnreadIndicator() {
    ...
    document.body.classList.toggle("app-has-chat-unread", chatUnread > 0);
    // browser tab title
    ...
    const stateKey = chatUnread + ":" + callUnread;
    if (n !== _lastUnread || stateKey !== _lastUnreadState) {
      _lastUnread = n;
      _lastUnreadState = stateKey;
      pushDesktopUnreadState(chatUnread, callUnread, !document.hasFocus());
    }
  }
  ...
  if (typeof window !== "undefined") {
    window.addEventListener("CorporateChatDesktopReady", () => {
      try { updateUnreadIndicator(); } catch (e) {}
    });
  }
  ...
  function renderChatList(chats) {
    ...
    list.innerHTML = visibleChats.map((c) => {
      ...
      const online = saved ? false : (other ? !!other.is_online : (c.members && c.members.some((m) => m.id !== State.me.id && m.is_online)));
      ...
      <div class="chat-item ${c.id === State.activeChatId ? "active" : ""} ${saved ? "saved-chat" : ""} ${presenceCls}" data-chat-id="${c.id}"${liveAttr}>
        ...
        <div class="chat-meta">
          ...
          <div class="chat-bottom">
            <span class="chat-last">${Drafts.get(c.id).trim() ? `<span class="draft-prefix">Черновик:</span> ${escapeHtml(Drafts.get(c.id).trim().slice(0, 60))}` : (c.last_message ? escapeHtml(c.last_message.slice(0, 60)) : "Нет сообщений")}</span>
  ...
  async function openChat(chatId) {
    ...
    try {
      ...
      renderChatArea();
      renderChatList(State.chats);
      API.markRead(chatId).then((r) => { if (State.activeChat && r && r.last_read_message_id) State.activeChat.last_read_message_id = r.last_read_message_id; refreshChatList(); }).catch(() => {});
    } catch (err) {
  ...
  function renderChatArea() {
    ...
    area.innerHTML = `
      <div class="chat-header">
        ...
        ${chat.type === "private" && !saved ? '<button class="icon-btn" id="web-call-btn" title="Позвонить через интернет">☎️</button>' : ""}
    ...
    input.addEventListener("input", () => {
      ...
      draftListTimer = setTimeout(() => renderChatList(State.chats || []), 350);
      sendTyping();
    });
  ...
  function messageReadInfo(m) {
    if (!m || m.sender_id !== State.me.id || !State.activeChat || !State.activeChat.members) return null;
  ...
  function updatePresence(userId, online, status) {
    ...
    State.chats.forEach((c) => c.members && c.members.forEach((m) => { if (m.id === userId) { m.is_online = online; m.status = online ? (status || "online") : "offline"; } }));
    ...
    overlay.innerHTML = `
      <div class="modal settings-modal">
        ...
        <div class="modal-body">
          ...
          <div class="settings-row"><div><div class="settings-label">Уведомления о звонках</div><div class="settings-sub">Входящие и пропущенные вызовы Windows/браузера</div></div>
