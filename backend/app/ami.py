...
field layouts. It maps PBX extensions to users by users.phone only. users.office is treated as cabinet/room, not a phone number.
"""
...
async def _find_user_by_extension(ext: str) -> User | None:
    """Find app user by PBX phone/extension.

    IMPORTANT: match only users.phone. users.office is a room/cabinet field,
    not a telephony extension. If office=401 and phone=204, a call to 401 must
    NOT create a missed call for that user.
    """
    n = _norm_phone(ext)
    if not n:
        return None
...
async def _is_office_only_number(value: str | None) -> bool:
    """True when value is a cabinet/office number, not a PBX phone.

    AD imports users.office from physicalDeliveryOfficeName. PBX can route a
    dialed cabinet number (e.g. 401) to the user's real phone (204). Such a
    dialed cabinet value must never be used as caller_number and must not create
    a missed call.
    """
    n = _norm_phone(value)
    if not n:
    ...
    async with async_session_maker() as db:
        users = (await db.execute(select(User))).scalars().all()
        matches_phone = any(_matches_number(n, u.phone) for u in users)
        if matches_phone:
            return False
...
async def _pick_caller(ev: dict[str, str], callee_ext: str) -> tuple[str, str]:
    ...
    for cand in candidates:
        ...
        if not cand or _same_number(cand, callee_ext):
            continue
        if await _is_office_only_number(cand):
            continue
        u = await _find_user_by_extension(cand)
        if u:
            return cand, (u.full_name or u.username or "")
    ...
    if await _is_office_only_number(fallback):
        fallback = ""
...
async def _record_event_facts(ev: dict[str, str]) -> dict[str, str]:
    ...
    # Outgoing call to city/mobile/external number: Channel is the caller's
    # internal endpoint (e.g. PJSIP/219), DestChannel is usually a trunk and
    # therefore has no user extension. Previously such events could be inverted
    # and shown as "980... called 219" / missed. Mark the internal endpoint as
    # originator and keep the external dialed number separately.
    if channel_ext and await _find_user_by_extension(channel_ext):
        if name == "dialbegin" or ev.get("DestChannel") or ev.get("Application") == "Dial":
            f.setdefault("originator", channel_ext)
        for cand in (ev.get("DestCallerIDNum"), ev.get("DestExten"), ev.get("Exten"), ev.get("ConnectedLineNum")):
            cand = _first(cand)
            cn = _norm_phone(cand)
            if not cn or _same_number(cn, channel_ext) or len(cn) <= 5:
                continue
            if await _find_user_by_extension(cn) or await _is_office_only_number(cn):
                continue
            f["external_destination"] = cn
            break
...
async def _create_call_from_facts(key: str, ev: dict[str, str], status: str = "ringing") -> tuple[CallEvent | None, User | None]:
    ...
    external_destination = f.get("external_destination") or ""

    # Outgoing call to an external/city number. The app user is the originator,
    # and the external number is the destination. Store it as outgoing so it is
    # never displayed as a missed incoming call from the city number.
    # Important: in outgoing external calls earlier facts can set callee to the
    # same internal endpoint as originator (Channel=PJSIP/204), so handle this
    # before creating an incoming missed row for the caller's own extension.
    if originator and external_destination and (not user or _same_number(callee, originator)):
        originator_user = await _find_user_by_extension(originator)
        if originator_user:
            out_status = "answered" if status == "answered" else ("ringing" if status == "ringing" else "ended")
            async with async_session_maker() as db:
                existing = (await db.execute(
                    select(CallEvent).where(CallEvent.linked_id == key, CallEvent.user_id == originator_user.id).order_by(CallEvent.id.desc())
                )).scalar_one_or_none()
                if existing:
                    ...
                    existing.extension = external_destination
                    existing.caller_number = _norm_phone(originator) or originator
                    existing.caller_name = originator_user.full_name or originator_user.username or ""
                    existing.raw = json.dumps(ev, ensure_ascii=False)[:4000]
                    await db.commit()
                    await db.refresh(existing)
                    _active[key] = existing.id
                    return existing, originator_user
                call = CallEvent(
                    user_id=originator_user.id,
                    extension=external_destination,
                    caller_number=_norm_phone(originator) or originator,
                    caller_name=originator_user.full_name or originator_user.username or "",
                    direction="outgoing",
                    status=out_status,
    ...
    if await _is_office_only_number(caller):
        if settings.AMI_DEBUG_EVENTS:
            logger.info("AMI call ignored: caller is office/cabinet number facts=%s event=%s", f, ev)
        return None, None
    if caller_user and caller_user.id == user.id:
        if settings.AMI_DEBUG_EVENTS:
            logger.info("AMI call ignored: caller and callee resolve to same app user facts=%s event=%s", f, ev)
        return None, None
...
async def _create_or_get_ringing(ev: dict[str, str]) -> tuple[CallEvent | None, User | None]:
    ...
    if await _is_office_only_number(caller):
        if settings.AMI_DEBUG_EVENTS:
            logger.info("AMI ringing ignored: caller is office/cabinet number caller=%s ext=%s event=%s", caller, ext, ev)
        return None, None
    if caller_user and user and caller_user.id == user.id:
        if settings.AMI_DEBUG_EVENTS:
            logger.info("AMI ringing ignored: caller and callee are same app user caller=%s ext=%s event=%s", caller, ext, ev)
        return None, None
...
async def _mark_ended_or_missed(ev: dict[str, str]) -> None:
    ...
    async with async_session_maker() as db:
        ...
        caller_user = await _find_user_by_extension(call.caller_number)
        self_leg = bool(caller_user and call.user_id and caller_user.id == call.user_id)
        if self_leg:
            call.direction = "outgoing"
        # Outgoing rows (internal->external/city or caller self-leg) must never
        # become "missed" for the caller. If nobody answered, it is simply ended.
        if call.direction == "outgoing":
            call.status = "answered" if answered else "ended"
        else:
            call.status = "answered" if answered else ("ended" if self_leg else "missed")
...
async def handle_ami_event(ev: dict[str, str]) -> None:
    ...
    if name in ("dialbegin", "newconnectedline") or csta_name == "deliveredevent" or (state == "ringing" and has_explicit_destination):
        ...
        if call and user and call.status == "ringing" and call.direction != "outgoing":
...
async def ami_originate(from_ext: str, to_ext: str, caller_id: str = "") -> dict[str, str | bool]:
    ...
    target_channel = settings.AMI_ORIGINATE_CHANNEL_TEMPLATE.format(ext=to_ext)
    ...
    contexts: list[str] = []
    tmpl = getattr(settings, "AMI_ORIGINATE_CONTEXT_TEMPLATE", "") or ""
    if tmpl:
        try:
            contexts.append(tmpl.format(from_ext=from_ext, to_ext=to_ext, ext=to_ext))
        except Exception:
            pass
    # Try configured context, then common per-extension contexts used by some PBXes.
    contexts = unique(contexts + [
        settings.AMI_ORIGINATE_CONTEXT,
        f"DLPN_DialPlan{from_ext}",
        f"DLPN_DialPlan{to_ext}",
    ])
    ...
    try:
        ...
        async def originate_by_context(ctx: str) -> dict[str, str] | None:
            await _send_action(
                writer,
                Action="Originate",
                Channel=channel,
            ...
            return await _read_ami_message(reader, timeout=8)

        async def originate_by_dial_app() -> dict[str, str] | None:
            # Last-resort fallback. First call the operator phone, then execute
            # Dial(target_channel) after answer. Context-based originate is
            # preferred because it uses the PBX dialplan correctly.
            await _send_action(
                writer,
                Action="Originate",
                Channel=channel,
        ...
        resp: dict[str, str] | None = None
        method = ""
        for ctx in contexts:
            resp = await originate_by_context(ctx)
            attempts.append({"method": "context", "context": ctx, "response": json.dumps(resp or {}, ensure_ascii=False)})
            if resp and str(resp.get("Response", "")).lower() == "success":
                method = f"context:{ctx}"
                break
            # If the only problem is missing extension in this context, try next context.
            if not (resp and "extension does not exist" in str(resp.get("Message", "")).lower()):
                break

        if not resp or str(resp.get("Response", "")).lower() != "success":
            # Optional fallback only. On some PBXes Dial(PJSIP/target) after
            # originating Channel=PJSIP/caller rings only the caller and looks
            # like a self-call. Keep it disabled unless explicitly enabled.
            if settings.AMI_ORIGINATE_DIAL_FALLBACK and attempts and all("extension does not exist" in a["response"].lower() for a in attempts):
                logger.info("AMI Originate all contexts failed (%s); retrying via Dial application", attempts)
                resp = await originate_by_dial_app()
                method = "dial_app"
                attempts.append({"method": "dial_app", "context": "", "response": json.dumps(resp or {}, ensure_ascii=False)})

        ok = bool(resp and str(resp.get("Response", "")).lower() == "success")
        error = "" if ok else ((resp or {}).get("Message") or (resp or {}).get("Response") or "AMI Originate failed")
        return {
            "ok": ok,
            "error": error,
            "response": json.dumps(resp or {}, ensure_ascii=False),
            "method": method or (attempts[-1]["method"] if attempts else ""),
