...
def _pick_auth_mechanism(user_identity: str):
    """Decide which ldap3 bind mechanism to use for the user password check.

    AD accepts two convenient password binds:
      * SIMPLE bind with a userPrincipalName (user@domain.com)
      * NTLM bind with a down-level name (DOMAIN\\user)
    The LDAP_AUTH_MECHANISM setting can force one of these.  KERBEROS/GSSAPI
    is intentionally NOT used here: a SASL/GSSAPI bind authenticates with the
    *server's* ticket cache, not the password the user just typed, so it can't
    validate user-supplied credentials.
    """
    mech = (settings.LDAP_AUTH_MECHANISM or "").strip().upper()
    if mech == "SIMPLE":
        return SIMPLE
    if mech == "NTLM":
        return NTLM
    # Auto: NTLM only makes sense for a down-level DOMAIN\\user identity.
    if "\\" in user_identity:
        return NTLM
    return SIMPLE
...
def _make_connection(user_identity: str, password: str) -> Connection:
    ...
    auth = _pick_auth_mechanism(user_identity)
...
def authenticate(username: str, password: str) -> LdapUser:
    ...
    is_admin = _in_any_group(groups, settings.LDAP_ADMIN_GROUP)
...
def _in_any_group(user_groups: list[str], targets: str) -> bool:
    """True if the user is a member of ANY of the given target groups.

    `targets` is a ';'-separated list. Each entry may be a full DN
    (CN=Foo,OU=...,DC=...) or just a group name (CN). Matching is
    case-insensitive; a bare name matches a user's group by its CN.
    """
    if not targets:
        return False
    user_dns = [g.strip().lower() for g in user_groups if g and g.strip()]
    user_cns = {group_cn(g).strip().lower() for g in user_groups if g and g.strip()}
    for raw in targets.split(";"):
        ...
        low = entry.lower()
        if "=" in entry:
            # looks like a DN -> exact DN match
            if low in user_dns:
                return True
            # also allow matching by the entry's CN (lenient)
            if group_cn(entry).strip().lower() in user_cns:
                return True
        else:
            # bare group name -> match against each user group's CN
            if low in user_cns:
                return True
    return False
