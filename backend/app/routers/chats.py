...
def _member_out(u: User, is_chat_admin: bool, last_read: int = 0) -> ChatMemberOut:
    return ChatMemberOut(
        ...
        is_chat_admin=is_chat_admin, last_read_message_id=last_read or 0,
        last_seen=u.last_seen, created_at=u.created_at,
    )


def _can_post(chat: Chat, my_membership) -> bool:
    """In a channel only chat-admins may post; everyone can post elsewhere."""
    if chat.type != "channel":
        return True
    return bool(my_membership and my_membership.is_admin)
...
def _build_chat_out(chat: Chat, current_user: User, member_rows, last_msg, my_membership, unread: int) -> ChatOut:
    ...
    for u, cm in member_rows:
        members.append(_member_out(u, cm.is_admin, cm.last_read_message_id))
    ...
    return ChatOut(
        ...
        can_post=_can_post(chat, my_membership),
        members=members,
    )
...
async def list_chats(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    # Sort: pinned/favorite chats first, then by most-recent activity.
    from datetime import datetime, timezone
    _epoch = datetime(1970, 1, 1, tzinfo=timezone.utc)

    def _key(c):
        ts = c.last_message_at or _epoch
        return (1 if c.is_pinned else 0, ts)
...
async def create_chat(
    ...
    if data.type == "group" and not perms["can_create_groups"]:
        raise HTTPException(status_code=403, detail="Ваша группа не может создавать группы")
    # Channels are organisation-wide announcement feeds — only app admins
    # (or group-admins, who are also app admins here) may create them.
    if data.type == "channel" and user.role != "admin":
        raise HTTPException(status_code=403, detail="Создавать каналы может только администратор")
...
async def toggle_pin_chat(chat_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    """Pin/unpin a chat to the top of the user's chat list (per-user favorite)."""
    cm = await _ensure_member(db, chat_id, user.id)
    cm.is_pinned = not cm.is_pinned
    await db.commit()
    return {"ok": True, "is_pinned": cm.is_pinned}
...
async def mark_read(chat_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if last and last != cm.last_read_message_id:
        cm.last_read_message_id = last
        await db.commit()
        # Tell the other members so their ✓ can turn into ✓✓ (read receipt).
