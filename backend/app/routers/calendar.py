...
async def _ensure_personal_calendar(db: AsyncSession, user: User) -> Calendar:
    """Return/create the user's personal calendar safely.

    Older builds could create duplicate personal calendars or calendars without
    a CalendarMember row. scalar_one_or_none() then raises MultipleResultsFound
    and both /calendar/calendars and /calendar start returning 500. We pick the
    oldest personal calendar and ensure membership exists.
    """
    cal = (await db.execute(
        select(Calendar)
        .where(Calendar.owner_id == user.id, Calendar.is_shared == False)  # noqa: E712
    ...
    member = (await db.execute(
        select(CalendarMember).where(CalendarMember.calendar_id == cal.id, CalendarMember.user_id == user.id)
    )).scalar_one_or_none()
    if member is None:
