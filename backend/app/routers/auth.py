...
async def _complete_sso_login(username: str, db: AsyncSession) -> TokenResponse:
    ...
    if user is None:
        ...
        if ldap_info and settings.LDAP_ADMIN_GROUP:
            from ..ldap_auth import _in_any_group
            if _in_any_group(ldap_info.get("groups", []), settings.LDAP_ADMIN_GROUP):
                role = "admin"
        ...
        if user.auth_source not in ("sso", "ldap"):
            ...
            changed = True
        # Keep admin role in sync with AD group membership on each SSO login.
        if ldap_info and settings.LDAP_ADMIN_GROUP:
            from ..ldap_auth import _in_any_group
            mapped = "admin" if _in_any_group(ldap_info.get("groups", []), settings.LDAP_ADMIN_GROUP) else "user"
            if user.role != mapped:
                user.role = mapped
                changed = True
        if changed:
