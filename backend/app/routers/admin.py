...
from ..models import AuditLog, CallEvent, Chat, ChatMember, DownloadEvent, Group, ManualContact, Message, User
from ..schemas import AdminStats, AuditLogOut, BroadcastRequest, ManualContactOut, TokenResponse, UserOut
...
class AdminCreateContactRequest(BaseModel):
...
def _local_day_bounds_utc(day=None) -> tuple[datetime, datetime]:
    """Return UTC bounds for a local Europe/Minsk calendar day."""
    tz = _local_tz()
    now_local = datetime.now(tz) if day is None else day
    start_local = now_local.replace(hour=0, minute=0, second=0, microsecond=0)
    end_local = start_local + timedelta(days=1)
    return start_local.astimezone(timezone.utc), end_local.astimezone(timezone.utc)


def _local_date_key(dt: datetime | None) -> str:
    if not dt:
        return ""
...
async def stats(db: AsyncSession = Depends(get_db), _: User = Depends(get_current_admin)):
    now = datetime.now(timezone.utc)
    today_start_utc, tomorrow_start_utc = _local_day_bounds_utc()
    ...
    messages_today = (await db.execute(select(func.count()).select_from(Message).where(
        Message.created_at >= today_start_utc,
        Message.created_at < tomorrow_start_utc,
...
async def analytics(db: AsyncSession = Depends(get_db), _: User = Depends(get_current_admin)):
    ...
    days = 14
    start_local_day = (now_local - timedelta(days=days - 1)).replace(hour=0, minute=0, second=0, microsecond=0)
    end_utc = (now_local.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)).astimezone(timezone.utc)
    start = start_local_day.astimezone(timezone.utc)
    labels = [(start_local_day + timedelta(days=i)).date().isoformat() for i in range(days)]
    msg_by_day = {d: 0 for d in labels}
    user_by_day = {d: 0 for d in labels}

    msg_rows = (await db.execute(select(Message.created_at).where(
    ...
    for dt in msg_rows:
        if dt:
            key = _local_date_key(dt)
            if key in msg_by_day:
                msg_by_day[key] += 1

    user_rows = (await db.execute(select(User.created_at).where(User.created_at >= start, User.created_at < end_utc))).scalars().all()
    for dt in user_rows:
        if dt:
            key = _local_date_key(dt)
...
@router.post("/users/contact", response_model=ManualContactOut)
async def create_manual_contact(
    data: AdminCreateContactRequest,
    db: AsyncSession = Depends(get_db),
    admin: User = Depends(get_current_admin),
):
    """Create a text-only entry for the Contact Book.

    IMPORTANT: this does NOT create a User account. The record is visible only
    in the Contact Book and will not appear in chats, right sidebar, login,
    permissions, etc.
    """
    ...
    await db.commit()
    await db.refresh(c)
    await _log(db, admin, "create_manual_contact", f"contact={c.full_name} phone={c.phone}")
    return ManualContactOut.model_validate(c)


@router.delete("/users/contact/{contact_id}")
async def delete_manual_contact(
    contact_id: int,
    db: AsyncSession = Depends(get_db),
    admin: User = Depends(get_current_admin),
):
    """Delete/deactivate a text-only contact-book entry.

    Only manual_contacts are affected. Real users / AD users are not touched.
    """
    c = (await db.execute(select(ManualContact).where(ManualContact.id == contact_id))).scalar_one_or_none()
    if not c:
        raise HTTPException(status_code=404, detail="Контакт не найден")
    c.is_active = False
    await db.commit()
    await _log(db, admin, "delete_manual_contact", f"contact={c.full_name} phone={c.phone}")
    return {"ok": True}
