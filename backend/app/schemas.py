...
class UserOut(BaseModel):
    ...
    status: str = ""           # "" | dnd | vacation (manual presence)
...
class StatusUpdate(BaseModel):
    # "" -> auto (clear), "dnd" -> Не беспокоить, "vacation" -> В отпуске
    status: str = Field(default="", pattern=r"^(|dnd|vacation)$")
...
class ChatOut(BaseModel):
    ...
    # Channels are read-only for non-admins. can_post tells the UI whether the
    # current user may post in this chat (always True except for channel readers).
    can_post: bool = True
...
class AdminStats(BaseModel):
    ...
    channels: int = 0
    sso_users: int = 0
    # time-series for dashboard charts (last 7 days)
    messages_per_day: list[TimePoint] = Field(default_factory=list)
    users_per_day: list[TimePoint] = Field(default_factory=list)
...
class BulkUserAction(BaseModel):
    user_ids: list[int] = Field(default_factory=list)
    action: str = Field(pattern=r"^(ban|unban|delete|make_admin|remove_admin|assign_group)$")
    group_id: Optional[int] = None  # only for assign_group
...
class CreateChannelRequest(BaseModel):
    ...
    description: str = Field(default="", max_length=500)
    member_ids: list[int] = Field(default_factory=list)
    add_all: bool = False  # subscribe every active user


class ChannelSubsRequest(BaseModel):
    member_ids: list[int] = Field(default_factory=list)
    add_all: bool = False
...
class ServerSettingsOut(BaseModel):
    ...
    retention_purge_attachments: bool = True
...
class ServerSettingsUpdate(BaseModel):
    ...
    retention_purge_attachments: Optional[bool] = None
